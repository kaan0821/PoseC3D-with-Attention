...
#----------------------------用的是BottleNeck----------------------------
        self.block, stage_blocks = self.arch_settings[depth]     #block=BottleNeck, blocks=(3,4,6,3)
        ...
        #--------------------stage在这里控制，i==0就是stage3，鉴于stage2其实根本不存在------------------/
        for i, num_blocks in enumerate(self.stage_blocks):               #enumerate输出就是（0，3）（1，4）（2，6）（3，3），我们实验用的 (4,6,3) 
            self.use_cbam = True                                         #当i=0,这个stage下的三个block全部都不要attention
            if i==0:
                self.use_cbam = False
